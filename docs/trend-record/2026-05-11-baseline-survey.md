...
### E. 다중 메신저는 위생 요건
OpenClaw(4 채널), Hermes(7 채널), SoloSquad(Discord/Slack/Telegram). 단일 채널 락인은 2026년 안티 패턴. → **SoloSquad v1.2.0+ 워크스페이스당 1 메신저 정책은 운영 단순화 목적이지만, 사용자 측에서는 채널 선택 자유가 유지됨. OK.**
...
| Hermes trajectory → skill | `experimental` | routine은 결과만 JSONL로 저장, tool-call trace는 없음 | 중간 | v1.7~v1.8 후보. claude-runner에 trace 옵션 추가 검토 |
