...
기능화(v1.9.0)하기 *전에*, 시뮬레이션이 예측력이 있는지부터 증거로 확인한다.
...
- **유효 시 → 대상 PRD:** `docs/prd/v1.9.0_virtual-market-demand-simulation.md`(기능화).
- **PRD 로 넘길 것:** 캘리브레이션된 `p_pay` 모델 · 통과 rubric(= 그 기능의 eval) · baseline 대비
  마진(효과 크기) · 익명화/누출 통제 규약.
- **사용자 판단 지점:** `final-result.md` 종합 판정 검토 → "유효" 승인 시 v1.9.0 착수.
