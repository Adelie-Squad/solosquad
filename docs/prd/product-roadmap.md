...
| 축 | v1.0 약속 | v1.x 진화 |
|---|---|---|
| 멀티 프로덕트 | multi-org·multi-repo·multi-user (v0.2~v0.8.0 완결) | v1.3 일정·메모 (지식 온톨로지와 같은 결) |
...
| 실험 중심 기획 | 4 디폴트 워크플로 + 5 기본 goal + 25 specialist + 워크플로우 메이커 | **실험 인프라 신설 (Amplitude 패턴 차용 — Q7 박제)** + 워크플로우/goal/루틴 고도화 |

**Leading indicator (Q1 박제 b)**: **24/7 자율 팀** 축이 leading. 측정 지표 = 사용자 메신저 대화 중 *작업 변환률*(대화→워크플로우/goal/루틴) · 자동 PR 성공률 · 자율 goal cycle 수 · dev_capability 활용도. 멀티 프로덕트(a)·실험 기획(c)은 자율 팀이 작동해야 의미가 있는 lagging 지표.
...
| 일정·메모·암묵지 → 지식 온톨로지로 통합 (v1.3+) | 외부 SaaS 종속 (Notion·Calendar는 MCP로만 연결, 데이터 원본은 사용자) |
...
- **포지셔닝**: **지식 온톨로지(v1.2)와 같은 결**. 사용자가 워크스페이스 위계에서 자신의 지식을 SoloSquad로 모아 SKILL/agent/워크플로우/goal/루틴의 성능을 지속 개선해 나간다. 모든 것을 *지식의 관점*에서 관리 — v1.2 지식 온톨로지가 그래프 백엔드 인프라 제공 → v1.3이 일정·메모를 그 위에 layer로 얹음
...
- **정의**: **하루를 넘는 장기 목표**. PM이 목표 달성을 위해 *워크플로우 + 루틴*을 조합해 자율 진행. 사용자가 중간에 워크플로우/루틴을 추가/변경/생략 지시 가능
...
- **암묵지 → SKILL 1차 source 박제 (Q2 b)**: **사용자 명시 슬래시** — `/save-as-skill <name>` 또는 PM 대화 중 자연어 "이거 SKILL로 저장해" 인식. 정확도 우선. v0.6 freq miner(반복 패턴 자동 감지)는 *backup* — 사용자가 명시 안 한 패턴은 *제안만* (자동 등록 안 함)
...
#### 3.2.7 작업 3분류 — 워크플로우 / goal / 루틴 (PM이 인지 + works 채널 스레드)
...
- 작업 3분류는 *상호 배타적 아님* — goal이 워크플로우·루틴을 *조합*해 자율 진행. 사용자가 goal을 새 생성하거나 진행 중 goal에 워크플로우/루틴 추가/변경/생략 지시 가능

**plan 문서**: 본 분류 자체는 product-roadmap 차원의 *비전*. 상세 구현(PM의 분류 휴리스틱·works 스레드 명명 규칙·README 가이드) 은 v0.9+ slot에서 다룬다 — 별도 vx.x.x plan 필요. (현재는 v0.3 PM 모드 + v0.4 goal-runner + v0.5 워크플로우 메이커 + v0.2.4 루틴이 각각 *분리 동작*)

#### 3.2.8 디폴트 루틴 정책 (2026-05-15 박제)
...
**영구 제거 (v0.8.5)**: Signal Scan (12:00) · Experiment Check (16:00) · Weekly Review (Sun 20:00) · `v06-retrospective-stats`. 사유: 분석 routine은 *디폴트로 강제하기엔 노이즈가 큼* + *비-디폴트로만 유지해도 결국 사용자 도메인 prompt가 있어야 의미 있어서 cron 슬롯을 차지할 가치 없음*. 도메인 분석은 사용자가 작성하는 워크플로우/goal로 표현. v0.2.4 박제 시점에 *제안*만으로 추가됐던 4건을 v0.8.5에서 자산·코드·스케줄 등록 모두 제거. 기존 워크스페이스의 `background_routines` 키는 forward-compat 위해 *읽기는 무시*로 유지. trajectory miner + freq miner는 별도 인프라 — `pm-compaction` routine 내부에서 호출, 독립 routine 아님.

**유지** (housekeeping, 디폴트 제공 외): `system-housekeeping` (v0.8.5 통합 — 단일 cron 00:00에 ① FTS5 cold archive rotation [구 archive-rotate, v0.6 §4] + ② 로그 retention 14일 [구 log-rotate, v0.8.3] 순차 실행, try/catch 격리). 이는 *시스템 무결성*에 필요 — 디폴트 분석 routine은 아니지만 인프라이므로 항상 활성. v0.8.5 이전 2건 분리는 통합 — UI 표시 1행, 결정적 함수는 2개 분리 유지(코드 동일).

**개인화 박제 (Q6 — 사용자별)**: 디폴트 3 루틴 모두 *사용자별 발송*. 각 사용자의 timezone·활성 설정에 따라 자기 봇이 자기 `works-<handle>` 채널에 발송.
...
- per-user 등록 모델은 *분리 슬롯* — 현재 활성 routine 5건이 모두 user yaml의 timezone·brief 시간으로 이미 동작
...
| `v0.8 후속 polish` | **메신저 UX polish (구 v0.8.4 명목 흡수)** + broadcast cross-user 작업 공유 feed + goal 1-active-per-org 모델 + 디폴트 루틴 3건 축소. 별도 minor/patch 버전 미고정, v0.8 단일 plan(`v0.8-multiuser-messenger.md` §3.8~§3.13 + §3.6 v2)에서 다음 patch가 흡수 구현 | `docs/plan/v0.8-multiuser-messenger.md` §3A | 기획 완료, 구현 대기 |
...
| `v0.9.x` | 안정화 + 자체 사용 검증 — 1주~1개월 self-dogfood, i18n 정책, `.github/ISSUE_TEMPLATE/`, trajectory 자동 등록 활성화(v0.8.3 4지표 통과 시), 디폴트 루틴 cleanup 마이그레이션 | `docs/plan/v0.9-self-use-and-i18n.md` (예정) | 기획 미수 |

### 5.2 정식 출시 마일스톤

| 버전 | 주제 | 비고 |
|---|---|---|
| **`v1.0.0`** ✓ 출시 (2026-05-21) | **정식 출시 (formal launch)** | `api-stability.md` 공개 약속 발효 + 42 CLI 명령 surface freeze + 진입 흐름 정합 2건 흡수 (`solosquad init` Step 1.5에 Claude login 흡수 + repo 등록 path-reference 단일화). 메신저는 Discord 단일, Slack은 post-v1.0 슬롯. 자세히 `docs/plan/v1.0-official-launch.md` |
...
| 버전 | 주제 | 문서 |
|---|---|---|
| **`v1.1.x`** | **Multi-Agent Team Architecture** — Hermes V2 5-layer (Gateway/Orchestrator/Team/Agent/Specialist) + Main+Specialist 분리 + Team Knowledge+OKR 공유 + Harness §7.5 4 권고 (Educational Nudge·Dependency Injection·Experiment Proposal·Board of Agents) + v1.x ideation Q1~Q7 흡수. **+ 2026-05-27 directive 흡수 (plan §21):** assets/ 폴더 unwrap → 루트 5 디렉토리 (`agents/main` + `agents/specialists`, `skills/`, `user/`, `team/`, `schedules/`), 팀 4축 재편 (chief/engineering/design/marketing), 기존 PM session → **chief 격상** (organization 위계 + 도메인 전문가 겸업), 25 specialist → 20 (5 병합), `routine` 어휘 → `schedule`. *L2~L5 만 — L1(메신저)은 v1.2* | `docs/prd/v1.1-multi-agent-team-architecture.md` |
| **`v1.2.x`** | **메신저 연결 (Discord 우선)** — L1 Gateway. Channel topology · Bot Identity Registry · 9-hop diagnostic · Forum Channel · Mention routing · Echo guard · Thread budget · v1.0.4 G+H+P+L+M+N+O 흡수. Slack 은 v1.2.x patch | `docs/prd/v1.2-messenger-connection-discord-first.md` |
| **`v1.3.x`** | **일정 관리 + 메모** — n잡 사용자 시간·기억 관리. 캘린더 통합·todo·노트 인프라. `docs/prd/v1.3-schedule-memo.md` 예정 | (기획 미수) |
...
| `v1.x` (archived) | 워크플로우 / goal / 루틴 *고도화* — Q1~Q7 ideation 7건 완전 흡수 후 본 슬롯은 *역사적 reference* 로 격하 (v1.1 plan §0 박제) | `docs/prd/v1.x-workflow-goal-routine-evolution.md` |
