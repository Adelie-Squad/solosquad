...
- **v1.x: 워크플로우/goal/cron 고도화** — 암묵지 → SKILL/워크플로우 자동화 강화. 별도 ideation 진행 중
...
- **포지셔닝**: **지식 온톨로지(v1.2)와 같은 결**. 사용자가 워크스페이스 위계에서 자신의 지식을 SoloSquad로 모아 SKILL/agent/워크플로우/goal/cron의 성능을 지속 개선해 나간다. 모든 것을 *지식의 관점*에서 관리 — v1.2 지식 온톨로지가 그래프 백엔드 인프라 제공 → v1.3이 일정·메모를 그 위에 layer로 얹음
