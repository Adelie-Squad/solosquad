...
| **`v1.3.2`** ⧖ 기획 | **Domain lifecycle managers — skill·workflow·goal·schedule 4개 자산의 생성/검토/검증/개선 정렬** | 각 1급 자산이 생성 경로는 있으나 검증/개선/lifecycle 이 제각각·불완전. v1.3.1 §7(skill-author→skill-manager) 흡수해 4개를 일관된 매니저 추상으로. P0: skill validate 강화 · workflow `validateWorkflow` 신설 · schedule 동적 레지스트리. 자세히 `docs/prd/v1.3.2-domain-lifecycle-managers.md` |
...
| **`v1.5.0`** | **커스터마이즈 ↔ Upstream Reconciliation** — 사용자가 워크스페이스 스킬/에이전트를 커스텀해도 SoloSquad 개선분을 안전히 받게 한다. *보존* 에만 최적화된 현 구조(tier-1 우선·`fs.existsSync` 가드·백업)에 *전파* 축 추가. M1: `~/.solosquad/` user-global overlay tier 실배선(oh-my-zsh custom/ 동형). M2: seed manifest + `update --reconcile` 3-way merge(copier 식). M3: health-gated auto-rollback(OpenClaw 식). M4: Claude Code 플러그인 정렬(PoC). 근거: `docs/trend-record/2026-06-16-customization-upstream-reconciliation.md` 6 레퍼런스 1차 조사 | 설계 초안 — `docs/prd/v1.5.0-customization-upstream-reconciliation.md` |
