...
| **`v1.2.7`** ✓ 출시 (2026-05-29) | **Bot spawn `--add-dir` for registered repos** | v1.2.6 이후 dogfood — 봇이 `cwd=<org>` 에서 spawn 되면 `C:\Dev\<repo>` 등 path-reference 등록된 외부 repo 에 접근 못 함 ("리포지토리 접근 권한 없음" + Chief 가 사용자에게 `/add-dir` 수동 실행 요청). v1.2.7 가 chief-runner spawn 에 `--add-dir <abs-path1> <abs-path2> ...` 자동 추가 — `<org>/repositories/*.yaml` 의 `path:` 필드를 모두 읽어 한 번에 전달. 워크스페이스 schema 변경 0, runtime 만 패치. Migration `1.2.6→1.2.7` = pure version bump |
