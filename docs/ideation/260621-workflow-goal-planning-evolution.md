...
### 2.0 현재 구현된 PMF 프레임워크·순서 (코드 직독, 2026-06-21)

> **이게 "현재 구현에 실제 사용된 프레임워크와 순서"다.** v1.3.5 는 이 순서를 기준선으로 삼는다.

**진입 게이트 — Six Forcing Questions** (gstack 차용, `agents/specialists/pmf-planner/SKILL.md`):
PMF 진입 전 자가검증. 6항목을 컨텍스트에서 답할 수 있어야 HARD GATE 통과, 아니면 회귀.
...
**핵심 시퀀스 — RO-PNA 6-Phase** (`skills/problem-definition/SKILL.md`, workflow `problem-definition`):

| Phase | 프레임워크 | RO-PNA 유형 | 산출 |
|---|---|---|---|
| P1 | **SCQA** (Situation·Complication·Question·Answer) | B 구조분해 | 문제 맥락 |
| P2 | **5-Whys** | C 원인추적 | 근본 원인 1문장 |
| P3 | **MECE** (후행/선행 문제 분해) | B 구조분해 | 분해 트리 |
| P4 | **TDCC** (후행지표·선행문제·기회·인과·미지) | C 원인추적 | 후보 매트릭스 |
| P5 | **XYZ 가설** (X%의 Y가 T 안에 Z, 왜냐하면 R) | D 가설실험 | 검증가능 가설 |
| P6 | **1-pager** → prd-writer 8-section | — | PRD |

**4개 구현 워크플로 (진입점별 순서, 모두 hard_gate except autoplan):**
- `problem-definition` — SCQA→5-Whys→MECE→TDCC→XYZ→1-pager (전 단계 pmf-planner). *가장 가벼운 진입.*
- `discovery-cycle` — discovery-synthesis → problem-definition → opportunity-tree(OST) →
  hypothesis → prd → wbs (pmf-planner→idea-scoper→feature-planner). *완전 자율 1패스.*
- `pmf-validation` — demand-reality → narrowest-wedge → interview-spec(researcher) →
  behavior-metric(data-analyst). *엄격 PMF 게이트.*
- `autoplan-pm` — pmf → business-strategist → policy-architect → data-analyst (hard_gate:false).
  *넓은 PM 체인.*

**공통 규약(pm_conventions):** anti_sycophancy · **hard_gate** · **post_labeling(사후 라벨링)** ·
**minimum_approaches: 2**.

> **현재 구현이 쓰는 RO-PNA 프레임워크:** SCQA·5-Whys·MECE·TDCC·XYZ (= B·C·D 유형) + OST +
> Six Forcing Questions. **아직 명시 단계로 안 쓰는 것:** A 고객발견형(IDEO 디자인씽킹·**JTBD**),
> 구글 CIRCLES, If-Then-Because. discovery-synthesis 가 JTBD 신호를 *데이터 마이닝*으로 일부
> 대체하나, **인터뷰/현장 기반 A 전단계**는 비어 있다(§5 격차).
...
## 7. PRD 착수 전 정해야 할 것 (결정 항목)

> 각 항목: **쟁점 → 옵션 → 트레이드오프 → 권장**. PRD 는 이 결정들을 입력으로 한다.

### D1. 표준 기획 단계 라인업·순서 (가장 중요)

- **쟁점:** 사용자 6단계(①아이디어 구체화 ②요구사항 분석 ③시장조사 ④데이터 분석 ⑤가설 ⑥개발
  요구사항)와 현재 구현(Six Forcing Questions 게이트 → SCQA→5-Whys→MECE→TDCC→XYZ→1-pager +
  discovery/OST)을 **하나의 정규 순서**로 합쳐야 한다. RO-PNA 유형 순서는 A고객발견→B구조분해→
  C원인추적→D가설.
- **옵션 A (RO-PNA 정렬, 권장):**
  `①아이디어 구체화(A: JTBD/디자인씽킹 + discovery-synthesis)` → `③시장조사(별도 리포트)` →
  `②요구사항 분석(B: SCQA·MECE·CIRCLES)` → `④데이터 분석(원인/지표)` → `⑤가설(C 5-Whys·TDCC →
  D XYZ·OST)` → `⑥개발 요구사항(spec-driven)` → `1-pager→PRD 8-section`. Six Forcing Questions 는
  진입 게이트로 유지.
- **옵션 B (현행 6-Phase 보존 + 양끝만 추가):** 앞에 A(아이디어/시장조사), 뒤에 ⑥(개발요구사항)만
  덧대고 중간 SCQA→…→XYZ 는 그대로. 변경 최소.
- **트레이드오프:** A 는 RO-PNA 의도(A 우선)에 충실·재설계 큼 / B 는 안전·기존 자산 재사용 최대지만
  순서가 "구조분해부터" 라는 현행 편향 유지.
- **권장: 옵션 A** — 단, 기존 6-Phase 스킬을 ②⑤ 안에 재사용(SCQA·MECE=②, 5-Whys·TDCC·XYZ=⑤).

### D2. 단계 강제(hard_gate) vs 유연(선택 진입)

- **쟁점:** 6단계를 순차 hard_gate 로 강제? RO-PNA 는 "순서는 가이드, 핵심은 4관점이 다 나오는 것".
- **옵션:** (i) 전 단계 hard_gate 순차(현 problem-definition 방식) / (ii) **OST 기반 선택 진입** —
  outcome→opportunity 에서 필요한 단계만 + "4관점(A/B/C/D) 커버리지 체크"를 게이트로 / (iii) 하이브리드:
  ②요구사항·⑤가설·⑥개발요구사항만 hard_gate, ①③④는 선택.
- **트레이드오프:** 강제=체계성·재현성↑ 유연성↓ / 선택=현실적·빠름, 누락 위험(완화: 관점 커버리지 체크).
- **권장: (iii) 하이브리드** — 핵심 3단계만 게이트 + RO-PNA "4관점 커버리지" 자가체크를 종료 조건에.

### D3. goal vs workflow 경계

- **쟁점:** 어느 단계가 workflow(1패스 결정적)이고 어느 게 goal(반복 수렴)인가.
- **옵션:** (i) **PRD 산출 전체 = 1 workflow**(1패스), 가설검증·시장 변화 추적만 goal / (ii) 시장조사·
  데이터분석을 goal cycle 로 반복(주간 리듬, OST assumption test), PRD 누적은 workflow.
- **트레이드오프:** goal 은 budget·termination·CONFIRMING 머신이 있어 *측정 가능한 반복*에 맞고, 1회성
  문서 누적엔 과함. 기획 산출(PRD)은 결정적 1패스가 자연.
- **권장: (i)** — 기획 파이프라인 = workflow, goal 은 "가설 metric 을 반복 검증"(⑤ 연장)에 한정.

### D4. 산출물 2종 — 시장조사 리포트 ↔ PRD 링크 규약

- **쟁점:** 시장조사는 별도 리포트, 나머지는 단일 PRD. 둘의 파일/참조 관계.
- **옵션:** 리포트 = `<org>/artifacts/market-research-<slug>-<date>.md`(또는 워크스페이스 자산), PRD
  의 evidence_refs[] 가 리포트를 경로로 참조. PRD 의 "시장" 섹션은 리포트 요약 + 링크만.
- **트레이드오프:** 분리=리포트 재사용·버전관리↑ / 인라인=단일 파일 편의. 사용자 요구가 "별도 리포트"라
  분리 확정.
- **권장:** 분리 + PRD evidence_ref 참조. 리포트는 §3.4 멀티에이전트 패턴(병렬 리서치+인용검증)으로 생성.

### D5. ⑥ 개발 요구사항의 깊이 (spec-driven 범위)

- **쟁점:** 기획 PRD 가 어디까지 개발 요구사항을 담나(예외/분기/사이드이펙트/수용기준). 개발은 기본
  agent 가 한다 했으므로 *경계* 가 필요.
- **옵션:** (i) PRD 에 "개발 요구사항" 섹션 — 예외/분기/사이드이펙트/수용기준(AC)을 spec-kit clarify
  식으로 능동 발굴해 명시, 구현은 기본 agent / (ii) 별도 spec 자산으로 분리.
- **트레이드오프:** PRD 통합=핸드오프 단순 / 분리=spec-driven 정통이나 자산 증가.
- **권장: (i)** — PRD 단일 관리(사용자 요구 ⑤)에 맞춰 PRD 섹션. clarify = 기존 open_questions[] 재사용.

### D6. 워크스페이스 skill·agent 갱신 범위·승인

- **쟁점:** CRUD 중 "필요 시 워크스페이스 위계 skill/agent 갱신" — 자동 생성/수정의 허용 범위·승인.
- **옵션:** (i) **재사용 우선 → 없으면 제안 후 사용자 승인 시 생성/수정**(v1.3.4 §G·cron-manager 동형,
  파괴적이면 확인) / (ii) 자율 생성(확인 없음).
- **트레이드오프:** 승인=안전·자산 난립 방지 / 자율=빠르나 위험.
- **권장: (i)** — 번들 불변, 워크스페이스 override 레이어, `asset validate` 게이트, 생성/수정 전 확인.

### D7. 기존 4 워크플로 정리

- **쟁점:** problem-definition / discovery-cycle / pmf-validation / autoplan-pm 를 v1.3.5 표준과 어떻게
  정합? 중복·혼란 위험.
- **옵션:** (i) **표준 1개(planning-pipeline) 신설 + 나머지는 그 위 프리셋/서브셋으로 재정의** /
  (ii) 4개 유지 + 6단계 매핑만 문서화.
- **트레이드오프:** 통합=일관성↑ 마이그레이션 필요 / 유지=안전하나 파편화 지속.
- **권장: (i)** — discovery-cycle 을 표준으로 승격·확장(A·⑥ 추가), 나머지는 프리셋.

### D8. A 고객발견 데이터 출처 (인터뷰 부재 fallback)

- **쟁점:** 1인 창업 초기엔 인터뷰 데이터가 없음. A(JTBD) 단계 입력 확보.
- **권장:** researcher 의 **desk research·시장 신호·경쟁 스캔**으로 1차 대체 + interview-script-author
  로 "추후 인터뷰 계획"을 산출(데이터 생기면 채움). 부재는 open_questions[] 로 명시.

### D9. 멀티 repo 기획 컨텍스트 비용

- **쟁점:** 전 repo 매니페스트+코드 주입 시 토큰 폭증.
- **권장:** v1.4.0 §6 가드 공유 — 매니페스트는 전체, 코드는 작업 scope repo 만. 기획은 주로 *읽기*라
  비용이 개발보다 작음.
