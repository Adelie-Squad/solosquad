...
>
> **2026-06-23 갱신(CMA 1st-party 추가).** Anthropic 공식 `anthropics/launch-your-agent`(Claude Managed
> Agents 빌드 스킬, Apache-2.0, 421★)을 전수 분석해 ⑴ **CMA = Anthropic 1차 *hosted* agent 하네스**
> (agent config 스키마·multiagent coordinator·Outcomes 자가채점·Memory store·Scheduled deployment 를
> *한 하네스에 융합* → 新 **B.6**), ⑵ **launch-your-agent 생애주기 스킬**(interview→stage/launch→
> grade/iterate→schedule = 우리 asset-manager G1~G6 의 1차 거울상 → **G** 보강), ⑶ **CMA Outcomes =
> 1차 자동 채점기**(rubric 3~6 binary·max_iterations·격리 채점 컨텍스트 → SkillOpt 가 요구한 "자동
> 검증기"의 1차 구현 → **E.2/E.4/E.5/F/G5**) 를 통합했다. CMA 는 본 조사 8개 포맷 중 **유일하게
> memory·grading·cron·multiagent 를 한 하네스에 융합한 1차 출처** — SoloSquad 전체 비전(스케줄되고·
> 자가채점하고·기억하는 squad)의 가장 가까운 1차 레퍼런스다. 출처는 platform.claude.com/docs 정본
> 인벤토리를 직접 fetch(1차).
...
| **Claude Managed Agents** (2026-06-23, **Anthropic 1차**) | API config(`agent_…`, **버전드**) | `name`(필수)·`description` | `model`(필수)·`tools`·`mcp_servers`·`skills`(≤20) | `multiagent:{type:coordinator, agents[]}`(roster, `{type:self}` 자기복제) | permission policy(`always_allow`/`always_ask`)·Outcome `max_iterations`(기본3/최대20)·multiagent **depth 1 only** |

**핵심 관찰:** 이제 8종(+Eve+CMA)이 **name/description/tools/model 4필드로 수렴**. A2A 만 "discovery 명함"(누가·무엇을·어디서·어떻게 인증)으로 한 단계 더 추상화. **description 은 단순 설명이 아니라 *위임 트리거*** — Claude Code 는 "include phrases like **'use proactively'**", OpenAI 는 `handoff_description`, 이 필드 품질이 자동 라우팅 정확도를 좌우. **(2026-06-23) Anthropic 1차 CMA 가 수렴 테제를 확정** — agent config 필수 = `name`·`model`, 선택에 `tools`/`mcp_servers`/`skills`/`multiagent`. 즉 우리 `SKILL.md` frontmatter 의 *API 정본*이며, 거기에 **버전 히스토리(config 변경마다 새 버전+핀+롤백)**를 1차 표준으로 더한다(상세 B.6).
...
## B.6 Claude Managed Agents (CMA) — Anthropic 1차 *hosted* 하네스 (2026-06-23 신규)
출처: `anthropics/launch-your-agent`(공식 빌드 스킬, Apache-2.0, 421★) + `cma-primitives.md`(platform.claude.com
/docs 정본 인벤토리). **본 조사 유일의 1차 *hosted* 하네스** — Messages API 가 agent loop 를 개발자가 돌리는
것과 대비, Anthropic 이 agentic loop·tool 실행·샌드박스 리눅스 컨테이너를 *서버측에서* 운영. 장기·비동기·
stateful 작업용. **4 코어 프리미티브:** **Agent**(재사용·**버전드** config 번들) · **Environment**(실행
컨텍스트, 세션마다 격리 컨테이너, 파일시스템 비공유) · **Session**(`sesn_…`, 대화이력+샌드박스 state,
checkpoint/resume) · **Events**(양방향 `user.*`/`agent.*`/`session.*`/`span.*`).

- **Agent config 스키마(1차, verbatim).** 필수 = `name`·`model`(예 `claude-opus-4-8` 또는 `{id,speed:fast}`).
  선택 = `system`·`tools`·`mcp_servers`·`skills`(≤20)·`multiagent`·`description`·`metadata`. **버저닝** =
  config 변경마다 새 버전(1→증가), ID(latest) 또는 `{type:agent,id,version:N}` 핀, update 시 현재 version
  동시성 가드. 배열 필드(tools/skills/mcp)는 **full replacement**, scalar 교체, metadata per-key 머지. Archive=
  read-only(기존 세션 계속, 신규 참조 불가). → **우리 `SKILL.md` frontmatter 의 API 정본** + **버전 히스토리**가
  비범위(G6)가 아니라 1차 *표준*임을 시사.
- **Multiagent = coordinator + roster (= SoloSquad Chief 의 1차 동형).** `multiagent:{type:coordinator,
  agents:[…]}`. roster = `{type:agent,id}`(latest)·`{id,version}`(핀)·**`{type:self}`(자기복제 spawn)**. 각
  sub-agent 가 **자기 session thread**(context 격리) — 샌드박스/파일시스템/vault 는 공유, **tools/MCP/context 는
  비공유**(= Cognition "share context" 경고의 부분 반영). **수치 가드의 1차 박제: depth 1 only(중첩 coordinator
  금지) · roster ≤20 · 동시 thread ≤25.** → **Anthropic 자신의 1차 하네스가 위임 depth 를 1 로 못박았다** —
  G1/G4 depth-cap 의 최강 선례(Claude Code subagent depth 5·OpenAI 10·LangGraph 25 에 더해 *coordinator
  depth=1*). 패턴 = parallelization·specialization·escalation. 휴리스틱 = "**build the single most valuable
  subagent first; the coordinator comes after**".
- **Outcomes = 1차 자가채점 루프 (★ v1.3.6 직격).** `user.define_outcome` 로 세션을 "대화"→"채점되는 작업"으로
  승격. `rubric`(필수, markdown, criterion 별 명시 체크, inline/file) + `max_iterations`(기본 **3**, 최대 20).
  채점은 **별도 격리 컨텍스트**(agent 선택과 분리)에서 pass/fail+설명 산출→다음 iteration 피드백. 결과 =
  `satisfied`/`needs_revision`/`max_iterations_reached`/`failed`/`interrupted`. rubric 설계 휴리스틱 = **3~6
  binary·job-specific·agent-checkable·bounded·"리뷰 없이 돌릴 자신이 드는가"**. → **SkillOpt 가 요구한 "자동
  검증기"의 1차 구현** — refine(G5)·held-out gate(skill-doc F)가 필요로 한 그 메커니즘을 first-party 로 제공.
- **Memory store = 1차 세션간 영속 (E.5 보강).** workspace-scoped 텍스트 문서 컬렉션(`memstore_…`), `/mnt/memory/`
  마운트, 일반 파일툴로 R/W, 세션 *생성 시에만* attach. **버전드**(immutable `memver_`) · `read_write`(기본) vs
  **`read_only`**(rw+untrusted = 프롬프트 인젝션으로 *미래 세션 오염* → 레퍼런스는 read_only) · 100kB/메모리·
  2000/store·8/session. **Dreams**(research preview) = memory+과거 1~100 transcript 를 읽어 **재구성된 새 output
  store** 생성(dedup·stale 교체·인사이트 표면화, 입력 불변) = **1차 메모리 consolidation 잡**. → Eve(메모리
  프리미티브 無)·agency-agents("learning memory"=마케팅)와 대비해 **CMA 는 1차 메모리 *메커니즘*을 가진 유일
  출처**. 단 write 게이트는 personal-agent-template 의 human-approved 보다 약함(파일툴 자유 write, read_only+
  인젝션 경고로만 방어).
- **Permission policy = 1차 HITL (E.4 보강).** 서버 실행 툴에 `always_allow`/`always_ask`. **기본: agent
  toolset=always_allow, MCP toolset=always_ask**(신규 MCP 툴 승인 강제). `agent.tool_use`→`requires_action`
  idle→`user.tool_confirmation`(allow/deny+deny_message). custom tool 은 클라가 게이트.
- **Scheduled deployment = 1차 native cron (호출 ③층의 1차 구현).** `depl_…`, `POST /v1/deployments`. 필수 =
  name·agent·environment·**`initial_events`**(user.message+선택 define_outcome)·`schedule:{cron, timezone}`.
  5-field POSIX cron·IANA tz·DST wall-clock·≤10s jitter·`upcoming_runs_at` 확인·**manual run 으로 cron 커밋 전
  테스트**·**상대날짜 필수**("today"/"last 14 days", 리터럴 금지). **sub-agent archived → 다음 run 실패 +
  deployment 자동 pause** = 그래프 무결성 붕괴 시 런타임 자동 차단(우리 G1 정적 검증의 런타임판). org당 1000개.

> **SoloSquad 함의(3중):** ⑴ **CMA config 스키마 = 우리 `SKILL.md` frontmatter 의 1차 정본** —
> name/model/tools/skills/multiagent 수렴 확정 + **버전 히스토리**가 표준임을 시사(G6 재고). ⑵ **CMA multiagent
> coordinator = Chief 의 1차 동형** — 단 **depth 1·roster≤20·thread≤25** 라는 *수치 가드*를 1차가 박았다
> (G1/G4 캘리브 근거). ⑶ **CMA Outcomes = refine(G5) 행동층이 기다리던 자동 검증기** — "검증기 있는 actor
> 부터"라는 우리 전제조건이 1차 메커니즘으로 충족 가능. **단 CMA 엔 정적 자산 검증·재사용 그래프가 없다**
> (런타임 API config 일 뿐) — 우리 차별화축은 *선언적 `SKILL.md` + validate 그래프 + org 레이어*.
...
- **(2026-06-23) "single agent first; multiagent only when earned" (Anthropic 1차, launch-your-agent).** "**One job, one context → single agent. Always start here.**" multiagent 는 ⑴ 진짜 분리 가능한 specialization, ⑵ 다수 항목 fan-out, ⑶ escalation tier 일 때만 earned — 그때도 "**build the single most valuable subagent first; the coordinator comes after**". = 우리 SRP·specialist>generalist 의 1차 재확인 + **착수 순서 규율**(coordinator 먼저 만들지 말 것). job 품질 신호 = "real judgment work(fetch-and-paste 아님) · 제공 가능한 입력 · checkable done state".
...
- **(2026-06-23) CMA(Anthropic 1차) multiagent coordinator depth = 1** (중첩 coordinator 금지, roster ≤20·동시 thread ≤25) + Outcome `max_iterations` 기본 3/최대 20. **Anthropic 자신의 hosted 하네스가 위임 depth 를 가장 보수적(1)으로 캡** — depth-cap 의 1차 최강 선례(B.6). + sub-agent archived 시 deployment **자동 pause**(런타임 그래프 가드).
...
- **(2026-06-23) CMA permission policy(Anthropic 1차)** — 서버 실행 툴에 `always_allow`/`always_ask`, **MCP toolset 기본 always_ask**(신규 MCP 툴 승인 강제) vs agent toolset 기본 always_allow. `requires_action` idle→`user.tool_confirmation`(allow/deny). 우리 비가역 액션 HITL 의 *기본값* 1차 선례 — 신뢰 낮은 외부(MCP)는 deny-by-default, 내장 툴은 allow(B.6).
...
**(2026-06-23) CMA(Anthropic 1차) — 메모리·채점을 *하네스에 융합*한 유일 출처.** Eve/personal-agent-template
가 메모리를 프레임워크 *밖* 앱 컨벤션으로 푼 것과 달리, **CMA 는 Memory store(1차 영속) + Outcomes(1차
자가채점) 를 하네스 프리미티브로 내장**(상세 B.6). 두 자가개선 층이 1차에서 모두 메커니즘으로 존재 — **②경험층
= Memory store + Dreams**(세션간 누적·재구성, 버전드, read_only/인젝션경고로 위생), **①행동층 = Outcomes rubric
루프**(격리 채점기 = SkillOpt 가 요구한 자동 검증기의 1차 구현). **교훈 보정:** CMA Memory 엔 personal-agent-
template 의 human-approved write 가 *없다*(파일툴 자유 write) — 우리는 **CMA Memory(메커니즘) + personal-agent-
template(승인 규율) 합성**이 최선(메커니즘은 CMA, 위생은 vercel-labs).
...
> **(2026-06-23)** ①행동층의 "자동 검증기 필요" 전제는 **CMA Outcomes(격리 rubric 채점기, max_iterations
> 3/20)** 가 1차로 충족 — refine held-out gate 를 자체 구현하기 전 CMA Outcomes 패턴(3~6 binary·격리
> 컨텍스트·`satisfied`/`needs_revision` 결과)을 그대로 차용 가능. 즉 v1.3.6 행동층은 *지금* 1차 레퍼런스가 있다.
...
7. **(2026-06-23) Anthropic 1차 CMA 가 수렴 테제를 확정 + 융합.** agent=버전드 config(name·model·tools·skills·multiagent) · multiagent=coordinator+roster(**depth 1**) · Outcomes=rubric 채점 · Memory store/Dreams · cron deployment 를 *한 하네스*에 통합. name/model/tools/skills 표준 + 위임 depth-cap(=1) + "single agent first"의 1차 근거.
...
**(2026-06-23) CMA 가 바꾼 그림.** 위에서 "시장이 행동층/경험층을 *분리*해 푼다"고 진단했으나 **반례 = CMA** —
Anthropic 1차 hosted 하네스는 **둘 다 한 하네스에 융합**(Outcomes=행동 채점, Memory+Dreams=경험). 즉 *통합
자체*는 더 이상 SoloSquad 만의 기회가 아니다 — **1차가 이미 통합 방향**. 따라서 우리 차별화는 통합이 아니라
**⑴ 선언적 `SKILL.md` 자산(CMA 는 런타임 API config) + ⑵ 정적 그래프 검증(validate, CMA 엔 없음) + ⑶ org
레이어 커스터마이즈(번들 불변+사용자 오버레이)**로 재설정해야 한다. 반대로 **메커니즘 차용은 적극적으로** —
Outcomes(자동 검증기)·Memory store(영속)·depth-cap(=1)·permission policy(MCP deny-by-default)는 1차를 그대로 베낀다.
...
**(2026-06-23) 1차 거울상 = `launch-your-agent` 생애주기 스킬.** Anthropic 공식 스킬이 agent 빌드를
**interview→plan → stage/launch → grade/iterate/eval → schedule** 4페이즈로 박제 — 우리 asset-manager
(G1 validate ~ G5 refine ~ lifecycle)의 1차 거울상이다. 직접 차용할 규율: ⑴ **v0 = 핵심 일만 하는 최소 agent**,
나머지는 NEXT-DIRECTIONS 에 *reason class(not possible / needs credential / scope)* 로 즉시 적재("cut" 아니라
"v1 예정"). ⑵ **"one job one context → single agent, always start here; multiagent only when earned"** +
**"가장 값진 subagent 먼저, coordinator 는 나중"**(Part C). ⑶ **rubric = per-run eval**(3~6 binary·job-specific·
agent-checkable) + **골든 케이스 1개로 v0, 나머지는 regression hold**(SkillOpt held-out 의 1차 운영판). ⑷
**상대날짜 필수**(스케줄 작업) · **mock connector in v0**(자격증명 없으면 schema-true outbox) · **manual run 으로
cron 검증 후 commit**. ⑸ **interview 는 폼 아닌 클러스터형 enumerable 선택지**(AskUserQuestion), 경계는 *맥락에서만*
짧게 — 우리 create/customize UX 직접 모델.
...
- refine(행동층) — §9.3 bounded-edit 루프를 SKILL.md body(역할 프롬프트)/agent-profile tone 에.
  **patch-mode add/delete/replace + held-out gate + rejected-edit buffer**(SkillOpt, skill-doc F). frontmatter
  그래프는 protected(refine 은 산문만). **자동 검증기 있는 actor 부터**(전제조건). **(2026-06-23) 그 검증기의
  1차 패턴 = CMA Outcomes**(격리 rubric 채점기, `max_iterations` 3/20, `satisfied`/`needs_revision` 결과) —
  held-out gate 자체 구현 전 rubric 3~6 binary·격리 컨텍스트 패턴을 차용 가능(B.6).
...
- **(2026-06-23) 재고 신호:** **actor 버전 히스토리**는 CMA 1차가 *표준*으로 보유(config 변경마다 새 버전+핀+
  롤백) — 비범위 유지하되 v1.4+ 우선순위 상향 후보. **per-agent 메모리 스코프**도 CMA Memory store(8/session·
  read_only/rw)가 1차 모델 제공 → G5 경험층과 합류 시 재평가.
...
- [ ] **순환 없음** — 위임 그래프에 무한 루프 없음(Kahn), depth 제한 있는가(1차 선례: **CMA coordinator depth 1**·Claude 5·OpenAI 10·LangGraph 25)
- [ ] **1차 정합** — frontmatter 가 CMA agent config(`name`·`model`·`tools`·`mcp_servers`·`skills`·`multiagent`)로 손실 없이 사상되는가
...
- [ ] **(자가개선) 경험층/메모리** — 고정 taxonomy · session-start 주입 · **human-approved write** ·
      ...
      (1차 메커니즘: CMA Memory store **read_only**+인젝션 경고 · Dreams consolidation)
- [ ] **(자가개선) 자동 검증기** — refine 행동층 전, **CMA Outcomes 식 격리 rubric 채점기**(3~6 binary·max_iterations)가 있는가(없으면 refine 금지)
...
- **(2026-06-23) Anthropic 1차 — Claude Managed Agents / launch-your-agent** — 공식 빌드 스킬 https://github.com/anthropics/launch-your-agent · CMA primitives & API 정본 https://platform.claude.com/docs/en/managed-agents/overview · 레포 내 `cma-primitives.md` · 생애주기/interview/examples `.claude/skills/launch-your-agent/{SKILL.md, references/interview.md, references/examples-bank.md}` · 블로그 "Building with Claude Managed Agents" · "get to production 10x faster"
