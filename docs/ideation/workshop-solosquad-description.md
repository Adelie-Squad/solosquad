...
> 문제정의 워크플로우·자산 4종), ⑵ **차별성**(단독 Claude/Codex 대비, OpenClaw·Hermes
...
> 서술은 2026-06-22 코드/문서 직독 근거이고, 외부 레퍼런스는 `README.md` References 표 +
> `docs/trend-record/` 베이스라인 근거. 차용/거절 결정은 CHANGELOG 출처를 병기한다.
...
├── agents/{main,specialists}/   에이전트 정의 (5 main + 20 specialist)
└── teams/{team}/                팀 멤버십(=데이터) · KNOWLEDGE · OKR
...
사고**한다는 것. SoloSquad 의 문제정의 코어는 RO-PNA PMF 게임의 6-Phase 다
(`skills/problem-definition/SKILL.md`):

```
P1. SCQA    Situation·Complication·Question·Answer 추출  (구조분해)
P2. 5-Whys  근본 원인 1문장                              (원인추적)
P3. MECE    후행/선행 문제 분해                          (구조분해)
P4. TDCC    후행지표·선행문제·기회·인과·미지            (원인추적)
P5. XYZ     검증 가능한 가설 (X%의 Y가 T안에 Z, 왜냐 R)  (가설실험)
P6. 1-pager → prd-writer 8-section synthesis             (PRD)
```
...
**4개 진입점 워크플로우:**
- `problem-definition` — 가장 가벼운 진입(6-Phase 단독)
- `discovery-cycle` — discovery→문제정의→OST→가설→PRD→WBS (완전 자율 1패스)
- `pmf-validation` — demand-reality→narrowest-wedge→interview→behavior-metric (엄격 게이트)
- `autoplan-pm` — pmf→business→policy→data (넓은 PM 체인, hard_gate 없음)

> **참가자 적용:** 자기 도메인의 "좋은 기획 절차"를 위 4규약(반증조건·하드게이트·사후라벨링·
> 복수접근)으로 SKILL.md 에 박제하라. 프레임워크는 RO-PNA 가 아니어도 된다 — *절차를 강제하는
> 구조*가 핵심이다.
...
SoloSquad 에는 재사용 가능한 자산이 4종 있고, 각각 멀티 repo·기획에 다른 역할을 한다:

| 자산 | 정체 | repo scope 결정 | 강점 | 한계 |
|---|---|---|---|---|
...
(출처: `260621-multi-repo-execution.md` §4.5)

**오케스트레이션 위계 — Chief → PM → specialist.**

```
사용자 ↔ 메신저 (#command-<handle>)
        ▼
   Chief (org 대면 오케스트레이터, 사용자가 시스템을 통제하는 핵심 통로)
...
- **교훈:** `npm publish` 는 비가역이다. 그래서 SoloSquad 는 **`--dry-run` 강제 + 4-docs
  pre-publish 게이트**(roadmap·architecture·manual KO/EN 4개 문서가 새 버전을 언급해야
  통과, `npm run docs-check` 가 `prepublishOnly` 에서 강제)를 도입했다 (`.claude/rules/
  git-workflow.md`). **비가역 작업 앞엔 자동 게이트를 세운다.**
...
### 8.5 용어 통일 — routine + schedule → cron (v1.3.3)

- **증상:** 같은 "예약 작업"을 built-in 은 *routine*, 사용자 작성은 *schedule* 로 불러
  혼란.
- **피봇:** 하나의 명사 **cron** 으로 통일 (코드·CLI·번들 디렉토리·온디스크 경로 전반의
  breaking rename + 마이그레이션 동반).
...
4. **입장 + 반증 조건 (anti-sycophancy).** "X 라고 판단합니다. Y 가 사실로 드러나면 입장
   바뀝니다." 아첨하는 기본값을 깨는 한 문장 (`problem-definition/SKILL.md`).
...
## 11. 현재 버전 토폴로지 — 에이전트·팀·스킬 인벤토리 (v1.3.4)

§2 가 *컨텍스트* 토폴로지였다면, 여기는 *행위자* 토폴로지다 — **5 main + 20 specialist, 4 team**
(`AGENTS.md` §Team Composition; `teams/{team}/composition.yaml` = 데이터).

```
                       사용자 ↔ #command-<handle>
                                  │
                        ┌─────────▼──────────┐
                        │  Chief (org 대면)   │  유일한 user-facing · 오케스트레이터
                        └─────────┬──────────┘
        ┌──────────────┬─────────┴────────┬──────────────────┐
     [product]      [design]         [engineering]        [marketing]
       PM           designer          engineer             marketer      ← 4 team supervisor (main)
        │              │                 │                    │
   pmf-planner     researcher        architect           brand-marketer
   feature-planner ux-designer       backend-engineer    gtm-strategist
   idea-scoper     ui-designer       fde                 performance-marketer
   business-strat.                   creative-frontend
   data-analyst                      data-engineer
   policy-architect                  cloud-admin
                                     qa-engineer
                                     security-engineer
```

**Main 에이전트 (5)** — Chief(org 대면) + 4 팀 supervisor:

| agent | team | 역할 |
|---|---|---|
| chief | — | org 대면 오케스트레이터. TRIAGE→DECOMPOSE→DISPATCH→SYNTHESIZE |
| pm | product | 자율 product thinking. 6 product specialist 오케스트레이션 |
| designer | design | 디자인 팀 supervisor (PM design doc → 디자인 작업) |
| engineer | engineering | 엔지니어링 팀 supervisor (design doc → 코드/인프라) |
| marketer | marketing | 마케팅 팀 supervisor (GTM/brand/performance) |

**Specialist 에이전트 (20) — 팀별:**

| team | specialists |
|---|---|
| **product (6)** | pmf-planner · feature-planner · idea-scoper · business-strategist · data-analyst · policy-architect |
| **design (3)** | researcher · ux-designer · ui-designer |
| **engineering (8)** | architect · backend-engineer · fde · creative-frontend · data-engineer · cloud-admin · qa-engineer · security-engineer |
| **marketing (3)** | brand-marketer · gtm-strategist · performance-marketer |

**Skill (19) — category별:**

| category | skills |
|---|---|
| problem/discovery | problem-definition · discovery-synthesis · interview-script-author |
| planning | hypothesis-design · experiment-design · opportunity-tree · prd-writer · prioritization · jobs-stories · lean-canvas · premortem · wbs-decomposition |
| orchestration | okr-writer · workflow-maker · triage |
| reflection | retrospective · skill-refinement · workflow-refinement · asset-review |

> **워크샵 포인트:** product 팀이 가장 깊다(6 specialist + 12 planning skill) — SoloSquad 가
> "기획 에이전트"인 이유. 디자인·개발·마케팅은 *기본 제공*으로 받쳐줄 뿐, 무게중심은 기획이다.

---

## 12. 작동원리 — 기본 workflow·goal·cron + 시나리오 플로우

**기본 workflow (5, `skills/workflow-maker/assets/workflows/`):**

| workflow | 순서 | hard_gate |
|---|---|---|
| `problem-definition` | SCQA→5-Whys→MECE→TDCC→XYZ→1-pager | ✅ |
| `discovery-cycle` | discovery-synthesis→문제정의→OST→가설→PRD→WBS | ✅ |
| `pmf-validation` | demand-reality→narrowest-wedge→interview→behavior-metric | ✅ |
| `autoplan-pm` | pmf→business→policy→data | ❌ |
| `weekly-retro` | metrics→retrospective→skill-refinement→workflow-refinement | ❌ |
...
> 빌트인 cron id `pm-compaction → chief-compaction` 으로 개명됐다.

**시나리오 플로우 — 메신저 1메시지가 자산으로 흐르는 경로:**
...
Chief · TRIAGE ── kind 분류 (triage skill): chat / workflow / cron / goal
   ...
   │
   ├─ workflow ─────► <org>/workflows/<id>/ 생성 (workflow-maker)
...
| 자산 | Create | Read | Update | Delete |
|---|---|---|---|---|
| **skill** | `/create`(대화) · `SKILL.md` 작성 | `asset list/show skill` | `SKILL.md` 편집 | 파일 삭제 |
| **agent** | `agents/specialists/<id>/SKILL.md` 작성 | `asset list/show agent` · `agent validate --graph` | `SKILL.md` 편집 | 파일 삭제 |
| **workflow** | 대화(Chief→workflow-maker) → `<org>/workflows/<id>/` | `workflow list/show` | `workflow.yaml` 편집 · `workflow set`(active) | snapshot revert |
| **goal** | `goal new <id>`(scaffold) | `goal list/show/status` | `goal.md` 편집 | 파일 · `goal stop`(실행 중단) |
| **cron** | `cron new`(대화 cron-manager) | `cron list/show/runs` | `cron edit/enable/disable` | `cron delete`(archive 기본, `--hard`) |

- **공통 검증 게이트:** `asset validate <kind>`(skill·agent·workflow·cron) — CI `validate-bundled`.
...
**SoloSquad 내부 (1차 — 직독)**
- `AGENTS.md` — Core Philosophy, 3-Layer Context, 8-layer JIT spawn, Team Composition,
  Crons, Multi-Session Execution, Handoff Protocol
- `skills/problem-definition/SKILL.md` — RO-PNA 6-Phase + pm_conventions(anti-sycophancy·
  hard_gate·post_labeling·minimum_approaches)
...
- `.claude/rules/git-workflow.md` — 4-docs pre-publish 게이트
