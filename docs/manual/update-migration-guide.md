...
이 문서는 **사용자용** 안내입니다. 마이그레이션 프레임워크 내부 구현은 `docs/v0.2.3-migration-process.md`를 참조.
...
현재는 `v1.x.x` 대역이며, v1 대역 내에서도 **구조 변경은 minor 자리에서 발생할 수 있습니다**(v0.2.0 예정).
...
### 4.2 v0.2.4 업데이트 시 주의사항

v0.2.4는 메신저 채널 구조와 라우틴 모델을 바꾸고, **Telegram 지원을 제거**합니다. 기존 사용자는 다음을 확인하세요.
...
**Telegram 사용자 (지원 제거)**:
- v0.2.4부터 `MESSENGER=telegram`은 작동하지 않음. 봇 시작 시 명확한 에러 메시지 출력.
- 전환 절차:
...
### 4.3 알려진 이슈 (v0.1.x → v0.2.0 점프 시)

- **`solosquad update` 가 마이그레이션 경고를 띄우지 않습니다.** 마이그레이션 감지 코드는 v0.2.0 에서 새로 추가됐기 때문에, v0.1.5 CLI 가 업데이트를 수행하는 순간에는 경고 로직이 존재하지 않습니다. v0.2.1 부터는 **모든 CLI 명령 시작 시 preAction 배너**로 불일치를 알립니다 (migrate/update/doctor 제외). v0.1.x 에서 올라오는 사용자는 **업데이트 직후 `solosquad migrate` 를 수동으로 한 번 실행**하세요.
- **`--dry-run` 플래그가 CLI 에 등록돼 있지 않을 수 있습니다** (v0.2.0 초기 빌드). v0.2.1 부터는 정식 등록됐으므로 `solosquad migrate --dry-run` 그대로 동작. 플래그 없이 `solosquad migrate` 만 써도 기본이 dry-run 입니다.
...
v0.1.2를 쓰다가 v0.3.0으로 직접 업데이트할 경우, 마이그레이션이 **체인**으로 순차 실행됩니다:
...
**주의:** 롤백 후에는 **복원된 버전에 맞는 CLI**를 쓰세요. v0.2.0로 마이그레이션했다가 v0.1.5 상태로 롤백했다면 CLI도 v0.1.5로 다운그레이드:
...
v0.2.0부터 Docker 이미지가 `${HOME}/.claude` 볼륨을 마운트하므로 호스트의 `claude login` 상태가 컨테이너에 공유됩니다. 호스트에서 다시 로그인:
...
**Q. 매번 업데이트해야 하나요?**
A. 아닙니다. 안정적으로 쓰고 있다면 급할 이유는 없습니다. 다만 **보안 hotfix**(예: v0.1.3 dotenv 누락)는 빠르게 적용 권장.
...
## 11. 마이그레이션 이후 — org/repo 관리 명령 (v0.2.1+)

v0.2.0 부터는 단일 트리 아래에서 여러 org/repo 를 운영할 수 있고, v0.2.1 부터는 `add org` / `add repo` / `sync` 정식 CLI 로 이 작업을 할 수 있습니다. 마이그레이션 이후 **처음 실행할 명령은 `solosquad sync`** 입니다 — legacy `.git` 을 정리하고 `.org.yaml` 을 실제 상태와 맞춥니다.
...
### 11.4 Legacy 정리 (v0.1.x → v0.2.0 마이그레이션 직후)

`1.1.x → 1.2.0` 마이그레이션 스크립트는 각 product 를 org 로 바꾸되, **`.git` 을 org 루트에 그대로 둡니다** (v0.1.x 에서 product=repo 였던 자취 — 강제 이동 시 사용자의 active 작업을 깨뜨릴 위험).
...
Normalize 후 봇은 자동으로 `<org>/repositories/<repo>/` 를 cwd 로 사용. Keep legacy 면 org 루트가 cwd (v0.2.1 의 `resolveOrgCwd` fallback).

### 11.5 cross-repo workflow 활용 (v0.2.1+)

v0.2.1 런타임은 워크플로우 `_status.yaml` 의 stage 에 `target_repo` 필드가 있으면 해당 stage 실행 시 그 repo 를 cwd 로 사용합니다. 여러 repo 에 걸친 작업 흐름을 하나의 workflow 로 조율할 수 있음:
...
stage 상태 전환 (pending → in_progress → completed) 은 현재 수동 편집. 자동화는 v0.3+ 로드맵.
