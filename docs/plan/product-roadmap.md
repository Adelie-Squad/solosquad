...
| 실험 중심 기획 | 4 디폴트 워크플로 + 5 기본 goal + 25 specialist + 워크플로우 메이커 | 워크플로우/goal/루틴 고도화 (별도 plan slot) |
...
- **모델 박제 (2026-05-15)**: org당 *여러 goal* 가능 (당연). 단 **한 조직에서 동시 active goal은 1개** — 다른 goal은 paused/queued 상태. `solosquad goal run <id>`는 active goal이 있으면 거부. 구현: `<org>/goals/.active-goal` 세마포어 또는 status yaml로 active 플래그 — *v1.x goal-runner 고도화* 슬롯에서 정식 구현 (`v0.4-autonomous-engine.md`에 후속 보강 항목으로 추가)
- **여전히 TBD**: multi-product cross-goal (1 goal이 여러 org reference) — n잡 사용자의 cross-product 자율 최적화 — v1.x ideation 계속
