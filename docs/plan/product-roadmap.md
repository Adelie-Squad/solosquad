...
**삭제** (디폴트에서 제거): Signal Scan (12:00) · Experiment Check (16:00) · Weekly Review (Sun 20:00) · `v06-retrospective-stats`. 사유: 분석 routine은 *디폴트로 강제하기엔 노이즈가 큼*. 사용자가 자기 도메인에 맞게 작성해야 의미. trajectory miner + freq miner는 *비-디폴트 인프라*로 유지 (자동 실행은 사용자가 활성화).

**유지** (housekeeping, 디폴트 제공 외): `archive-rotate` (v0.6 §4 — FTS5 cold archive 일일 rotation) · `log-rotate` (v0.8.3 — 로그 retention 14일). 이 둘은 *시스템 무결성*에 필요 — 디폴트 분석 routine은 아니지만 인프라이므로 항상 활성.
...
**구현 영향**:
- `assets/routines/signal-scan.md`·`experiment-check.md`·`weekly-review.md`·`v06-retrospective-stats.md` 제거 또는 *비-디폴트 examples/*로 이동
- `src/scheduler/routines.ts` 디폴트 등록 목록에서 제거 + per-user 등록 모델로 전환 (각 user yaml의 routine 설정 읽기)
- 마이그레이션: 기존 워크스페이스의 cron 등록은 그대로 두되 (사용자가 능동적으로 활성화한 것이라면 유지) — v0.9 migration script에서 사용자 확인 prompt 후 disable 권장
- README + master-guide: 디폴트 루틴 표 갱신 (5종 → 3종 사용자별)
