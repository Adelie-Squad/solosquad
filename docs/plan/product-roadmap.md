...
| **`v1.0.4`** ✓ 출시 (2026-05-22) | **Discord config.yaml 자동 생성 + Slack author-guard 통째 cleanup** | v1.0.3 Bug D fix 자가비판 박제 — `syncGuildProductMapping` 의 *서버명 휴리스틱* 만 제거하고 *file-existence early-return* 분기는 그대로 둬서 사용자가 v1.0.3 설치 후에도 *"No product linked to this server"* 받음. v1.0.4 = (G) load-or-empty + auto-write 패턴으로 진짜 fix — 봇이 자기 정보로 `<org>/discord/config.yaml` *첫 시작 시 자동 작성*. (H) v1.0.2 약속의 마무리 — Slack 어댑터 author-guard 호출 제거 + `src/bot/author-guard.ts` + `test/author-guard.test.ts` 통째 삭제. 7번째 누적 fix — v1.0.3 plan §6 *반복 패턴* 의 3번째 변형 (*silent file-bail*) 박제. 자세히 `docs/plan/v1.0.4-messenger-config-auto-create.md` |
