...
- **v0.8.4 polish** (기획 완료): `@bot help` 메신저 내부 매뉴얼 + `solosquad init --verify` e2e 검증 + sticky welcome 메시지 + `messenger ensure-channels` 복구 + wizard 단계 6→4 축소 — `v0.8.4-messenger-ux-polish.md`
...
- **목표 / 도달 수준 TBD (Q.GR)**: 현재는 1 org × 1 goal. multi-product cross-goal 필요?
...
| **`v0.8.4`** | **Messenger UX Polish** — `@bot help` 메신저 내부 매뉴얼 + `solosquad init --verify` e2e 검증 + sticky welcome 메시지 + `messenger ensure-channels` 채널 부재 복구 + wizard 단계 6→4 축소 + assets/messenger-manual/ 콘텐츠 source | `docs/plan/v0.8.4-messenger-ux-polish.md` | 기획 완료, 구현 대기 |
