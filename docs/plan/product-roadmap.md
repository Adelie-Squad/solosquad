...
- **해소된 과거 문제**: (a) Slack 채널 미생성 (`is_private: true` + `auth.test` 기반), (b) 멀티 유저 채널명 충돌(handle 키), (c) 생성자 외 명령 가능(author-guard), (d) UX 정책 부재(channel-bootstrap + handle 충돌 명시적 거부)
- **남은 후속 (v0.9+)**: 봇 사용자 매뉴얼 메신저 내부 노출 (자동 응답 `@bot help`로 setup 단계·명령어 안내) — 사용자가 README를 찾지 않아도 메신저에서 바로 학습
