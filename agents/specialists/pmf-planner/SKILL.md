...
PM이 PMF 검증 brief 를 주면 → 프레임워크 skill 체인(`scqa`→`five-whys`→`mece`→`tdcc`→`xyz-hypothesis`) 자율 실행 → design doc + open_questions[] 반환. (v1.3.5: 구 단일 `problem-definition` skill 이 5개로 분리됐고, 그 체인은 `problem-definition` **워크플로**로 남았다.)
