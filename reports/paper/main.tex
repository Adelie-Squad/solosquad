...
\begin{abstract}
Founders of \emph{0-to-1} products---new offerings with no launch history---are
urged to validate demand before building, but real validation (interviews,
fake-door tests, A/B) is costly. A tempting shortcut is a \emph{virtual market} of
synthetic personas that role-play the target customer and decide whether to engage,
pay, or churn. We ask whether such a simulation predicts \emph{real} conversion,
and answer with a blind, leakage-controlled backtest: five documented historical
validation episodes, anonymized so the model cannot recall their outcomes, judged
by 200 population-grounded persona agents each and scored against the real result.
Naive one-shot simulation is \emph{anti-predictive}---real successes receive lower
simulated conversion than real failures ($\Delta=-0.46$)---and this failure is
invariant to the decision framing (stated, revealed, graded), to the aggregation
statistic (mean through top decile and maximum), and to a longitudinal
reformulation; it recurs on two live products, predicting $\sim$14\% paid
conversion where the team targets 5\%. The bias is only partly correctable:
leave-one-out calibration lifts directional accuracy from $0.40$ to $0.64$ over 14
cases, but the sim--real map is non-monotonic---some over-hyped products are
correctly rejected---so a perfect five-case result does not survive expansion. We
conclude that synthetic-persona demand sims must not be used raw for 0-to-1
validation and only partly recover under calibration, and we release the blind
backtest as a reusable validity gate.
\end{abstract}

\section{Introduction}
\label{sec:intro}
AI coding lets founders ship \emph{0-to-1} products---new offerings with no sales
history---faster than ever, yet few sustain value. The recommended hedge is
\emph{demand validation}: before building, test whether the target customer will
actually convert, using fake-door tests, concierge MVPs, waitlists, landing-page
pricing, or Wizard-of-Oz prototypes. All are costly for a solo builder, so a
\emph{virtual market}---synthetic personas that role-play the customer and decide
whether to show interest, pay, or churn---is an attractive substitute; recent work
supplies population-grounded persona datasets~\citep{nemotron} and multi-agent
engines~\citep{mirofish}. Our scope is exactly this setting: pre-launch,
paid-conversion validation for novel products, the regime where real evidence is
scarcest and simulation most tempting. The unanswered question is epistemic:
\emph{can a persona simulation predict conversion that holds in the real world?} We contribute (i) a \textbf{blind, leakage-controlled backtest
...
\emph{novel} ones~\citep{persona_microdata}, and demographic persona conditioning
can \emph{degrade} rather than improve alignment~\citep{persona_reliability}---
echoing our finding that target-segment conditioning does not help. The documented
...
paid subscription tier, and \textbf{P2}, an open-source multi-agent planning tool
for solo founders. Predicted paid conversion is $0.14$ (P1) and
$0.10$ (P2), essentially
...
Pre-launch products without real labels (P1, P2) still require a real anchor
...
For 0-to-1 demand validation, a synthetic-persona virtual market is not a drop-in
substitute for real evidence: used raw it \emph{inverts} the signal, and no
reformulation we tried repairs it, because one-shot persona willingness tracks
broad appeal rather than the niche, sustained conversion that separates real
winners from losers. The constructive half of the finding is that the bias is
\emph{systematic}: a light calibration against a handful of real outcomes recovers
part of the signal ($0.40\rightarrow0.64$)---enough to \emph{rank} or \emph{screen}
candidate ideas, not to estimate absolute demand. We therefore see three concrete
uses. \textbf{(i) A calibrated screening feature} inside AI planning tools for founders:
a cheap simulation shortlists ideas that a small real fake-door test then confirms,
cutting the cost of the first validation loop.
\textbf{(ii) A diagnostic}: the simulation's confident false positives are exactly
the broadly-appealing-but-doomed concepts, so a high raw score is a useful
\emph{caution} flag. \textbf{(iii) A benchmark}: our blind, leakage-controlled
backtest is a reusable validity gate against which any persona-simulation vendor
can be scored before it informs a real decision. Closing the residual gap calls for
a larger labeled corpus, revealed-behavior traces rather than stated intent, and
calibration that models the non-monotonic, regime-dependent sim--real map---the
agenda our results make concrete.
...
logs for all framings---the full research workflow and experiment cycles---will be
released as part of an open-source AI-planning project upon acceptance; author
identity is withheld for review.
% Camera-ready: released in SoloSquad, https://github.com/Adelie-Squad/solosquad (logs: reports/logs/)
