...
import { migration } from "../src/migrations/scripts/1.1.0-to-1.2.4.js";
...
test("1.1.0→1.2.4 — detect() fires on 1.1.0", async () => {
  const ws = tempWorkspace("1.1.0");
  assert.equal(await migration.detect(ws), true);
});

test("1.1.0→1.2.4 — detect() fires on 1.1.0.<extra> (4-segment legacy)", async () => {
  const ws = tempWorkspace("1.1.0.1");
  assert.equal(await migration.detect(ws), true);
});

test("1.1.0→1.2.4 — detect() does NOT fire on 1.0.x", async () => {
  const ws = tempWorkspace("1.0.4");
  assert.equal(await migration.detect(ws), false);
});

test("1.1.0→1.2.4 — detect() does NOT fire on 1.2.4 (idempotent)", async () => {
  const ws = tempWorkspace("1.2.4");
  assert.equal(await migration.detect(ws), false);
});

test("1.1.0→1.2.4 — plan() includes version bump", async () => {
  const ws = tempWorkspace("1.1.0");
  const plan = await migration.plan(ws);
  const bump = plan.steps.find(
    (s) => s.kind === "update" && (s.to ?? "").includes("workspace.yaml.version=1.2.4"),
  );
  assert.ok(bump, "version-bump step missing");
});

test("1.1.0→1.2.4 — plan() includes discord workspace policy seed when missing", async () => {
  const ws = tempWorkspace("1.1.0");
  const plan = await migration.plan(ws);
...
test("1.1.0→1.2.4 — plan() warns about Chief name + owner_only=false (upgrade-safe)", async () => {
  const ws = tempWorkspace("1.1.0");
  const plan = await migration.plan(ws);
  assert.ok(
...
test("1.1.0→1.2.4 — apply() bumps version + seeds messenger.discord", async () => {
  const ws = tempWorkspace("1.1.0");
  ...
  assert.equal(after.last_migrated_to, "1.2.4");
...
test("1.1.0→1.2.4 — apply() preserves an existing messenger.discord block (idempotent)", async () => {
  const ws = tempWorkspace("1.1.0");
...
test("1.1.0→1.2.4 — apply() never clobbers a pre-existing workflows/problem-definition/workflow.yaml", async () => {
...
test("1.1.0→1.2.4 — verify() succeeds after apply", async () => {
  const ws = tempWorkspace("1.1.0");
  await migration.apply(ws);
...
test("1.1.0→1.2.4 — verify() reports version mismatch when apply did not run", async () => {
  const ws = tempWorkspace("1.1.0");
  const v = await migration.verify(ws);
