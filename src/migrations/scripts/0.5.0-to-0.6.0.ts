...
function ensureOrgLayerStubs(workspace: string, orgSlug: string): void {
  ...
  if (!fs.existsSync(profilePath)) {
    const templatePath = path.join(getAssetsDir(), "templates", "agent-profile.yaml");
...
// ---------------------------------------------------------------------------
// Redestination report (§6 — assets/templates/migration-redestination-report.md)
// ---------------------------------------------------------------------------
...
function writeRedestinationReport(opts: WriteRedestinationReportOpts): void {
  const templatePath = path.join(
    getAssetsDir(),
    "templates",
    "migration-redestination-report.md"
  );
...
- Auto-applied (role): {AUTO_ROLE}
- Auto-applied (domain): {AUTO_DOMAIN}
- Human review required: {HUMAN_REVIEW}
- Skipped: {SKIPPED}
- Budget-stopped: {BUDGET_STOPPED}
- Total: {TOTAL}

LLM cost: \${LLM_COST_USD} / \${BUDGET_CAP_USD}
