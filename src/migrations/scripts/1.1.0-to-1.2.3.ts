...
/**
 * v1.1.0 → v1.2.3 — Discord auto-connect + Chief identity + owner-only gate +
 ...
 *
 *   1. **Bump** workspace.yaml.version to 1.2.3.
 *
 ...
 *     are untouched.
 *
 * Idempotent: detect() returns false on 1.2.3; apply() guards every seed
 * with existsSync.
 */
...
export const migration: Migration = {
  ...
  to: TARGET,
  description:
    "v1.2.3 — Discord auto-connect + Chief identity (chief_name) + owner-only gate + works-handle task hub + problem-definition workflow seed. Bumps workspace version, seeds workspace.yaml.messenger.discord defaults (owner_only=false preserves v1.0.2 behavior), and copies problem-definition workflow to each org.",
  ...
  async plan(workspace: string): Promise<MigrationPlan> {
    ...
    if (ws) {
      steps.push({
        ...
        description: "Bump workspace version to 1.2.3",
      });
