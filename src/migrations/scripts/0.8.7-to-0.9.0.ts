...
/**
 * v0.8.7.x → v0.9.0 — Workspace ↔ Repository relationship redesign (model B).
 *
 * Per `docs/plan/v0.9-workspace-repo-relationship.md`. v0.9.0 ships:
 ...
 *
 * Idempotent: re-running on 0.9.0 is a no-op (`detect()` returns false).
 */
...
export const migration: Migration = {
  ...
  to: TARGET,
  description:
    "v0.9.0 workspace↔repo redesign (model B path-reference) — version bump only (no schema changes)",
  ...
  async plan(workspace: string): Promise<MigrationPlan> {
    ...
    if (ws) {
      steps.push({
        ...
        description: "Bump workspace version to 0.9.0",
      });
    }
    return {
      steps,
