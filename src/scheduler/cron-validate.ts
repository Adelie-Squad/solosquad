...
export function validateCronDef(
  ...
  if (def.at) {
    ...
    f.warn({ code: "CRON_TOO_FREQUENT", id, field: "cron", message: "every-minute cron — confirm this is intended (min-interval guard)" });
  }
  ...
  f.errorIf(!def.channel || def.channel.length === 0, { code: "CRON_CHANNEL_MISSING", id, field: "channel", message: "channel is required" });
