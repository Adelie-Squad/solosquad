...
## 작성 표준 (이 skill 의 핵심 — 번들로 점진공개)
skill 을 쓰거나 고칠 때 아래 3개 reference 를 **그때 읽어** 적용한다(본문엔 요지만):

- **`references/authoring-guide.md`** — description 공식, 본문 절차/명명패턴(Gotchas·Verification 게이트·
  anti-rationalization), 정량 한도, **번들 구조(scripts/references/assets) 규칙**, 점진공개.
- **`references/frontmatter-fields.md`** — 필드별 load-bearing vs decorative **감사**. 핵심 원칙:
  *모든 커스텀 필드는 동작을 구동하거나 검증기가 강제해야 한다 — 둘 다 아니면 추가하지 마라.*
- **`references/eval-recipe.md`** — **description 트리거 eval**(20쿼리×3런, 임계 0.5, train/val split) +
  **output 품질 A/B**(with/without_skill, 비용 delta). 개선의 "좋아졌다"를 측정으로 증명.

**3대 요지(reference 없이도 기억할 것):**
...
3. **description 먼저** — `authoring-guide.md` §2 공식으로 작성. **이게 가장 중요**하니 여기 시간을 쓴다.
4. **본문 작성(대화)** — `authoring-guide.md` §3 의 명명 패턴에서 *필요한 것만* 골라 채운다. 번들이
   필요하면 §5 로 scripts/references/assets 를 구분해 둔다. 안티-시코펀시: 최소 2개 접근 비교 후 추천.
5. **eval 설계** — `eval-recipe.md` §1 로 description 트리거 셋(20쿼리)을, 검증가능 output 이면 §2 A/B 를 둔다.
6. **검토·검증** — `asset-review` 로 품질 검토 후 `solosquad skill validate` 게이트 통과 확인.
...
2. **표준 대조** — 위 3 reference 기준으로 무엇이 어긋났는지 짚는다(description 약함? decorative 필드?
...
- **LLM 품질 리뷰**(다자산 공통) = `asset-review`. skill-manager 는 *작성/개선*을 소유, 단발 리뷰는 위임 가능.
