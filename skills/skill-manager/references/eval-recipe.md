...
>
> **누가 채점하나 (v1.3.6 §3.3/§3.5 — 핵심):** 별도 API 클라이언트가 아니라 **세션에서 도는 Claude**(Chief
> 가 skill-manager/`skill-refinement` 를 실행)다. **판단**("이 쿼리가 skill 을 켜나?" "with-skill 이 더 나은가?"
> "어떤 edit 을 할까?")은 도는 Claude 가 **Task 로 judge 서브에이전트를 띄워** 한다. **결정적 부기**(trigger-rate
> 집계·train/val split·A/B delta)는 `src/analyze/eval-corpus.ts`, **자가개선 gate**(held-out 채택·edit 예산·
> rejected buffer)는 `src/analyze/refine-gate.ts` 가 맡는다 — 판단은 사람-LLM, 산술은 코드.

## 1. description 트리거 eval — 운영 (골격 = core §8 ①)
- **쿼리 셋:** 20개 = should 8–10 + should-NOT 8–10. should-NOT 엔 **near-miss 네거** 필수(키워드 겹치되
  다른 skill 이 맞는 케이스 — 예: CSV-분석 skill 에 "엑셀 *수식* 수정"). 형식 = `{query, should}[]`(eval-corpus.ts).
- **실행:** 각 쿼리 ×3런 → judge 서브에이전트(Task)가 "켜졌나" 판정 → `scoreTrigger(results)` 로 trigger-rate
  집계(should>0.5 / should-NOT<0.5).
- **overfit 방지:** `splitTrainVal(queries, 0.6, seed)` 로 train 60% / val 40% **고정 split**, ~5 iteration,
  **val 최고본 선택**("best ≠ last"). 실패 쿼리 키워드 직박 금지 → 상위 개념을 잡는다. 매 라운드 1024자 재확인.
...
위 두 eval 이 채점기가 되면 다음을 얹는다. **Rollout·Reflect·Edit 작성은 도는 Claude**가, **gate 산술은
`src/analyze/refine-gate.ts`**(`runRefineGate`/`acceptEdit`/`RejectedEditBuffer`)가 한다:
**Rollout(현 skill 실행, scored 궤적) → Reflect(성공/실패 *분리* 반영) → Edit(span 단위 patch, `Lt=4`/floor 2
예산 = `DEFAULT_EDIT_BUDGET`) → Gate(`acceptEdit`: held-out 점수가 best 를 *엄격* 초과할 때만 채택).**
- **rejected-edit buffer:** 거부된 편집+점수하락을 `RejectedEditBuffer` 에 기록 → 재제안 차단(네거티브 신호).
