...
description: primitive(skill·agent·workflow·goal·cron) 정의를 정적 validate 이후 LLM 품질 관점으로 검토. 모호한 description·불명확한 위임 경로·측정 불가 exit_criteria·cadence-prompt 불일치를 지적. 사용자나 Chief 가 "리뷰해줘" 요청 시 호출.
schema_version: 2
tier: leader
team: _skill
category: core
...
- **id/경로**: 검토 대상 (예: `product/product-manager`, `flows/weekly-retro/workflow.yaml`)
