...
**버전은 항상 3자리 `vN.N.N` (4자리 금지).** 핫픽스도 별개가 아니라 **다음 patch**(예:
`1.3.8` → `1.3.9`)이며, PRD↔버전 1:1 그대로 **자체 PRD** 를 갖되 `prd` 스킬의 **핫픽스 PRD 양식**
(증상→근본원인→영향→해결책→복구절차→회고)으로 가볍게 쓴다. 복수 기능 동시 ship 은 버전을
